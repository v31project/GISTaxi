...
class Krugozor_Pagination_Helper
{
    ...
    <ul class="pagination">
        <? if ($this->view_first_page_label && $this->manager->getCurrentSeparator() && $this->manager->getCurrentSeparator() != 1): ?>
            &nbsp;<li><a<?=$this->createInlineCssClassDeclaration('first_page_class', 'normal_link_class')?> title="<?=$this->html['first_page_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getPageName()?>=1&amp;<?=$this->manager->getSeparatorName()?>=1<?=$this->createFragmentIdentifier()?>"><?=$this->html['first_page_anchor']?></a></li>&nbsp;
        <? endif; ?>

        <? if ($this->view_previous_block_label && $this->manager->getPreviousBlockSeparator()): ?>
            <li><a<?=$this->createInlineCssClassDeclaration('previous_block_class', 'normal_link_class')?> title="<?=$this->html['previous_block_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getPageName()?>=<?=$this->manager->getPageForPreviousBlock()?>&amp;<?=$this->manager->getSeparatorName()?>=<?=$this->manager->getPreviousBlockSeparator()?><?=$this->createFragmentIdentifier()?>"><?=$this->html['previous_block_anchor']?></a></li>&nbsp;
        <? endif; ?>

        <? if($this->manager->getPreviousPageSeparator() && $this->manager->getPreviousPage()): ?>
            <li><a<?=$this->createInlineCssClassDeclaration('previous_page_class', 'normal_link_class')?> title="<?=$this->html['previous_page_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getPageName()?>=<?=$this->manager->getPreviousPage()?>&amp;<?=$this->manager->getSeparatorName()?>=<?=$this->manager->getPreviousPageSeparator()?><?=$this->createFragmentIdentifier()?>"><?=$this->html['previous_page_anchor']?></a></li>&nbsp;
        <? endif; ?>

        <? foreach($this->manager->getTemplateData() as $row): ?>
            <? if($this->manager->getCurrentPage() == $row["page"]): ?>
                <li<?=$this->createInlineCssClassDeclaration('active_link_class')?>><a><?=$this->createHyperlinkAnchor($row)?></a></li>
            <? else: ?>
                <li><a<?=$this->createInlineCssClassDeclaration('normal_link_class')?> href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getSeparatorName()?>=<?=$row["separator"]?>&amp;<?=$this->manager->getPageName()?>=<?=$row["page"]?><?=$this->createFragmentIdentifier()?>"><?=$this->createHyperlinkAnchor($row)?></a></li>
            <? endif; ?>
        <? endforeach; ?>

        <? if($this->manager->getNextPageSeparator() && $this->manager->getNextPage()): ?>
            &nbsp;<li><a<?=$this->createInlineCssClassDeclaration('next_page_class', 'normal_link_class')?> title="<?=$this->html['next_page_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getPageName()?>=<?=$this->manager->getNextPage()?>&amp;<?=$this->manager->getSeparatorName()?>=<?=$this->manager->getNextPageSeparator()?><?=$this->createFragmentIdentifier()?>"><?=$this->html['next_page_anchor']?></a></li>
        <? endif; ?>

        <? if($this->view_next_block_label && $this->manager->getNextBlockSeparator()): ?>
            &nbsp;<li><a<?=$this->createInlineCssClassDeclaration('next_block_class', 'normal_link_class')?> title="<?=$this->html['next_block_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getSeparatorName()?>=<?=$this->manager->getNextBlockSeparator()?><?=$this->createFragmentIdentifier()?>"><?=$this->html['next_block_anchor']?></a></li>
        <? endif; ?>
        ...
        <? if ($this->view_last_page_label && $this->manager->getLastSeparator() && $this->manager->getCurrentSeparator() != $this->manager->getLastSeparator()): ?>
            &nbsp;<li><a<?=$this->createInlineCssClassDeclaration('last_page_class', 'normal_link_class')?> title="<?=$this->html['last_page_title']?>" href="<?=$self_uri?>?<?=$queryString?><?=$qs?><?=$this->manager->getPageName()?>=<?=$this->manager->getLastPage()?>&amp;<?=$this->manager->getSeparatorName()?>=<?=$this->manager->getLastSeparator()?><?=$this->createFragmentIdentifier()?>"><?=$this->html['last_page_anchor']?></a></li>
        <? endif; ?>
    </ul>
<?php
        $str = ob_get_contents();
